...
- **Reader** ✅ — Непрерывный мониторинг Telegram-каналов (polling каждые 10 минут)
  ...
  - Применяет фильтры по include/exclude ключевым словам
...
tag_filters:                        # Универсальный механизм фильтрации
  jobs:                             # Фильтры для тега "jobs"
    ...
    exclude_keyword_combinations:   # ОПЦИОНАЛЬНО (AND-исключения)
...
| `tag` | string | ✓ | Тег указывается как ключ внутри `channels` и связывает канал с `tag_filters` |
...
Канал с одним тегом
    ...
    └→ Для каждого сообщения проверяем фильтры тега канала:
        • Если tag="jobs" → apply_tag_filters(text, tag_filters["jobs"])
    └→ Сохраняем, если сообщение проходит фильтры тега
...
**Важное изменение:** теперь каждый канал связан ровно с одним тегом. Конфигурация с `tags: [jobs, news, learning]` больше не поддерживается.
...
Reader поддерживает unit-тесты фильтрации с внешними тест-кейсами.

**Где лежат кейсы:**
- `app/reader/tests/filter_cases.yml`
...
jobs_filter_cases:
  should_pass:
    - |
      Текст вакансии, который должен пройти фильтр
  should_fail:
    - |
      Текст вакансии, который должен быть отфильтрован
```

`exclude_keyword_combinations` работает как AND внутри одной комбинации и как OR между комбинациями:
- если все ключевые слова из одной комбинации найдены в тексте, пост исключается;
- если хотя бы одного слова из комбинации не хватает, исключение по этой комбинации не срабатывает.

**Как добавить новый кейс:**
1. Открой `app/reader/tests/filter_cases.yml`
2. Добавь текст в `should_pass` или `should_fail`
3. Сохрани файл (Python-код тестов менять не нужно)

**Запуск тестов:**
```bash
python3 -m unittest discover -s app/reader/tests -p "test_*.py"
```
...
def should_save_post(text: str, channel_tag: str, tag_filters: dict) -> bool:
    """
    Для тега канала проверяем фильтры:
...
2. Добавь канал под новым ключом `channels.my_new_tag`
